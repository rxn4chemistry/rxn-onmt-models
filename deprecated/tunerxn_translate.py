...
@click.option(
    "--model_path",
    type=str,
    help="Model path - defaults to the model_path finetuned by IBM",
)
...
def translate(
    ...
    translate_standardize(
        ...
        output_file_path=standardized_file,
        fragment_bond="~",
    )
    ...
    print(
        f"Post-processing... {input_file_path} and {translation_file} -> {output_file_path}"
    )
