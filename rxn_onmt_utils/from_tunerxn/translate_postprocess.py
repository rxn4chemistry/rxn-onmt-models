...
def postprocess_predictions_and_confidences(
    ...
    vals = [
        (record_from_df[f'{output_tag}_{i}'], record_from_df[f'confidence_{i}']) for i in indices
    ]

    # Create SinglePrediction instances from it, and keep only the valid ones
    predictions = [
        SinglePrediction(tokenized_smiles, confidence) for tokenized_smiles, confidence in vals
    ]
    valid_predictions = [prediction for prediction in predictions if prediction.smiles is not None]
    ...
    return [(prediction.smiles, prediction.confidence) for prediction in collapsed_predictions]
...
def translate_postprocess(
    ...
    for r in records:
        ...
        for i, (smiles, confidence
                ) in itertools.zip_longest(indices, vs, fillvalue=(INVALID_PRODUCT, 0.0)):
