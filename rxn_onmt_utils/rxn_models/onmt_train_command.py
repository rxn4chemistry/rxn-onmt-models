...
class OnmtTrainCommand:
    ...
    def __init__(
        self, command_type: RxnCommand, no_gpu: bool, data_weights: Tuple[int, ...], **kwargs: Any
    ):
    ...
    def _build_cmd(self) -> List[str]:
        ...
        for arg in ONMT_TRAIN_ARGS:
            ...
            if self._command_type not in arg.needed_for:
                ...
                if arg_given:
                    raise ValueError(f'"{arg.key}" value given, but not necessary for {command}')
                continue
    ...
    @classmethod
    def train(
        cls, batch_size: int, data: PathLike, dropout: float, heads: int, layers: int,
        learning_rate: float, rnn_size: int, save_model: PathLike, seed: int, train_steps: int,
        transformer_ff: int, warmup_steps: int, word_vec_size: int, no_gpu: bool,
    ...
    def continue_training(
        cls, batch_size: int, data: PathLike, dropout: float, save_model: PathLike, seed: int,
        train_from: PathLike, train_steps: int, no_gpu: bool, data_weights: Tuple[int, ...]
    ) -> 'OnmtTrainCommand':
        return cls(
    ...
    @classmethod
    def finetune(
        cls, batch_size: int, data: PathLike, dropout: float, learning_rate: float, rnn_size: int,
        save_model: PathLike, seed: int, train_from: PathLike, train_steps: int, warmup_steps: int,
        no_gpu: bool, data_weights: Tuple[int, ...]
    ) -> 'OnmtTrainCommand':
        return cls(
