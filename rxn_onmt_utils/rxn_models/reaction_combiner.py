...
from rxn_chemutils.reaction_smiles import (
    ReactionFormat, parse_any_reaction_smiles, to_reaction_smiles
)
...
class ReactionCombiner:
    ...
    def combine(self, precursors: List[str], products: List[str]) -> Iterator[str]:
        ...
        """
        precursor_multiplier, product_multiplier = self._get_multipliers(precursors, products)

        # repeat itemwise the elements: https://stackoverflow.com/a/45799308
        precursor_iterator = chain.from_iterable(zip(*repeat(precursors, precursor_multiplier)))
        product_iterator = chain.from_iterable(zip(*repeat(products, product_multiplier)))

        for precursor_string, product_string in zip(precursor_iterator, product_iterator):
    ...
    def _try_to_reaction_smiles(self, precursor_string: str, product_string: str) -> str:
        ...
        if self.standardize:
            reaction_equation = sort_compounds(canonicalize_compounds(reaction_equation))

        return to_reaction_smiles(reaction_equation, reaction_format=self.reaction_format)

    def _get_multipliers(self, precursors: List[str], products: List[str]) -> Tuple[int, int]:
