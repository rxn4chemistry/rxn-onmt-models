...
def test_different_output_formats():
    ...
    assert list(
        ReactionCombiner(reaction_format=ReactionFormat.STANDARD).combine(precursors, products)
    ) == ['CC.O>>CCO', 'CCC.O>>CCCO']
    assert list(
        ReactionCombiner(reaction_format=ReactionFormat.STANDARD_WITH_TILDE
                         ).combine(precursors, products)
    ) == ['CC~O>>CCO', 'CCC~O>>CCCO']
    assert list(
        ReactionCombiner(reaction_format=ReactionFormat.EXTENDED).combine(precursors, products)
