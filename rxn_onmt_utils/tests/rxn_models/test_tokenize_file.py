...
from rxn_onmt_utils.rxn_models.tokenize_file import tokenize_classification_file, classification_file_is_tokenized, \
    classification_string_is_tokenized, tokenize_class, detokenize_class, file_is_tokenized
...
def test_tokenize_classification_file():
    with tempfile.TemporaryDirectory() as temporary_dir:
        ...
        tokenize_classification_file(temporary_path / 'a.txt', temporary_path / 'a.txt.tokenized')
        assert load_list_from_file(temporary_path / 'a.txt.tokenized'
                                   ) == ['1 1.2 1.2.3', '0', '2 2.11 2.11.4', '11 11.0 11.0.23']

        # Invalid classes present
        dump_list_to_file(['1.2.3.5', '0', '2.11.4', '11.0.23'], temporary_path / 'b.txt')
        tokenize_classification_file(temporary_path / 'b.txt', temporary_path / 'b.txt.tokenized')
        assert load_list_from_file(temporary_path / 'b.txt.tokenized'
                                   ) == ['', '0', '2 2.11 2.11.4', '11 11.0 11.0.23']
