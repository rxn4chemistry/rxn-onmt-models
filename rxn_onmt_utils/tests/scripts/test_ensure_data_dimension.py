...
def test_ensure_data_dimension():
    ...
    with tempfile.TemporaryDirectory() as temporary_dir:
        ...
        dump_list_to_file(
            ['A.B.C', 'D.E.F', 'G.H.I', 'L.M.N', 'O.P.Q'], temporary_path / 'precursors.txt'
        )
        dump_list_to_file(['A', 'B', 'C', 'D', 'E'], temporary_path / 'products.txt')

        files = (temporary_path / 'precursors.txt', temporary_path / 'products.txt')
        ensure_data_dimension(txt_files=files, output_dir=temporary_dir, max_dimension=2)
        ...
        assert load_list_from_file(temporary_path / 'chunk_0' / 'products.txt') == ['A', 'B']
        assert load_list_from_file(temporary_path / 'chunk_0' / 'precursors.txt'
                                   ) == ['A.B.C', 'D.E.F']
        assert load_list_from_file(temporary_path / 'chunk_1' / 'products.txt') == ['C', 'D']
        ...
        assert load_list_from_file(temporary_path / 'chunk_2' / 'precursors.txt') == ['O.P.Q']
...
def test_ensure_data_dimension_one_chunk_only():
    ...
    with tempfile.TemporaryDirectory() as temporary_dir:
        ...
        files = (temporary_path / 'precursors.txt', temporary_path / 'products.txt')
        ensure_data_dimension(txt_files=files, output_dir=temporary_dir, max_dimension=6)
...
def test_ensure_data_dimension_different_length():
    ...
    with tempfile.TemporaryDirectory() as temporary_dir:
        ...
        dump_list_to_file(['A.B.C', 'D.E.F', 'G.H.I', 'L.M.N'], temporary_path / 'file1.txt')
        dump_list_to_file(['A', 'B'], temporary_path / 'file2.txt')

        files = (temporary_path / 'file1.txt', temporary_path / 'file2.txt')

        with pytest.raises(ValueError):
            ensure_data_dimension(txt_files=files, output_dir=temporary_dir, max_dimension=3)
